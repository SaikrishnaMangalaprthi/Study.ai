...
export default function HealthTracker() {
  ...
  // FIX: single debounce timer ref — prevents one API call per button click
  ...
  async function loadHistory() {
    try {
      ...
      console.error("Error loading health history");
    } finally {
      setLoading(false);
    }
  }

  async function handleSaveLog(dataToSave) {
    try {
      await api.post("/health/", dataToSave);
      setSuccessMsg("Health metrics saved!");
      loadHistory();
  ...
  // FIX: debounce saves — waits 800ms after last click before hitting the API
  const debouncedSave = (data) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => handleSaveLog(data), 800);
  };

  const adjustWater = (amount) => {
    setLog(prev => {
      const updated = { ...prev, water_ml: Math.max(0, prev.water_ml + amount) };
  ...
  const adjustSleep = (amount) => {
    setLog(prev => {
      const val = Math.max(0.0, Math.round((prev.sleep_hours + amount) * 10) / 10);
      const updated = { ...prev, sleep_hours: val };
      debouncedSave(updated);
      return updated;
    });
  };
  ...
  const adjustExercise = (amount) => {
    setLog(prev => {
      const updated = { ...prev, exercise_min: Math.max(0, prev.exercise_min + amount) };
      debouncedSave(updated);
      return updated;
    });
  };
  ...
  async function handleCalculateBMI() {
    if (!heightCm || heightCm <= 0) { setErrorMsg("Please enter a valid height."); return; }
    if (!log.weight_kg || log.weight_kg <= 0) { setErrorMsg("Please enter and save your weight first."); return; }
    try {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Maintain a healthy mind and body to optimize study productivity</p>
      </div>
        ...
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6 shadow-xl space-y-6">
          ...
          <div className="bg-black/30 p-4 rounded-xl space-y-3">
            ...
            <div className="w-full bg-white/5 rounded-full h-1.5">
              <div className="bg-blue-400 h-1.5 rounded-full transition-all" style={{ width: `${Math.min((log.water_ml / 2000) * 100, 100)}%` }} />
            </div>
          ...
          <div className="bg-black/30 p-4 rounded-xl space-y-3">
            <div className="flex justify-between items-center">
              <div>
                <span className="text-sm font-bold text-gray-200">😴 Sleep</span>
            ...
            <div className="w-full bg-white/5 rounded-full h-1.5">
              <div className="bg-indigo-400 h-1.5 rounded-full transition-all" style={{ width: `${Math.min((log.sleep_hours / 8) * 100, 100)}%` }} />
            </div>
            <div className="flex gap-2">
          ...
          <div className="bg-black/30 p-4 rounded-xl space-y-3">
            <div className="flex justify-between items-center">
              <div>
                <span className="text-sm font-bold text-gray-200">🏃 Exercise</span>
            ...
            <div className="w-full bg-white/5 rounded-full h-1.5">
              <div className="bg-emerald-400 h-1.5 rounded-full transition-all" style={{ width: `${Math.min((log.exercise_min / 30) * 100, 100)}%` }} />
            </div>
            <div className="flex gap-2">
              <button onClick={() => adjustExercise(5)}
          ...
          <div className="bg-black/30 p-4 rounded-xl space-y-4">
            <div className="space-y-1">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => handleSaveLog(log)}
          ...
          <div className="bg-black/30 p-4 rounded-xl space-y-4">
            ...
            {bmiData && (
              <div className="border-t border-white/5 pt-3 text-center space-y-1.5">
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Your BMI</p>
                ...
                <p className="text-[10px] text-gray-500 mt-1">Based on {bmiData.weight_kg} kg</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto max-h-[350px] pr-1">
              ...
              {history.length === 0 && (
                <div className="text-center py-16 text-gray-500">
                  ...
                  <p className="text-[10px]">No logs yet. Start logging above.</p>
                </div>
              )}
            </div>
          )}
